...
tree.plot.meta.heat <- gheatmap(tree.plot.meta, clades[, c("cluster", "Lineage"),  drop = FALSE], width = 0.05, color = "white", offset = 0.01) +
  scale_fill_manual(values = lineage_colors, name = "") + theme(legend.position = "none")
...
tree.plot.div.heat <- gheatmap(tree.plot.div, clades[, c("Lineage", "cluster"),  drop = FALSE], width = 0.05, color = "white", offset = 0) +
  scale_fill_manual(values = lineage_colors, name = "")
